...
tf_tall              = 0x00000002
tf_short             = 0x00000004
tf_child             = 0x00000006 #exploit the fact that tall, short, and child are all exclusive to squeeze another flag out of our byte
tf_fat               = 0x00000008
...
#balance_mod_change_ryantheskinny
...
from module_constants import companions_begin,companions_end

def append_npc_copies(troops):
  troop_copy = deepcopy(troops)
  
  #find npc's by stripping out "trp_"
  npc_name_len = len(companions_begin)
  first_npc = companions_begin[4:npc_name_len]
  first_idx = find_troop(troops,first_npc)
  
  npc_name_len = len(companions_end)
  last_npc = companions_end[4:npc_name_len]
  last_idx = find_troop(troops,last_npc)
  
  if (first_idx == -1):
    print "Error with append_npc_copies: Unable to find " + first_npc + " at index " + first_idx
  elif (last_idx == -1):
    print "Error with append_npc_copies: Unable to find " + last_npc + " at index " + last_idx
  else:
    for troop_idx in range(first_idx,last_idx):
      troop_copy[troop_idx][0] = 'copy_'+troops[troop_idx][0]
      troops.append(troop_copy[troop_idx])
